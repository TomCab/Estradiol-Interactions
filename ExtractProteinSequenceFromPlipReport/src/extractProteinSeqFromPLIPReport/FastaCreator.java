...
public class FastaCreator {
    ...
    public static void preprocessChains(ArrayList<ArrayList<String>> trimmedMegaList,
        ...
        for(int i = 0; i < trimmedMegaList.size(); i++) {
            ...
            if(threshold < 1) {
                ...
                for(int j = 0; j < trimmedMegaList.get(0).size(); j++) {
                    ...
                    // If some <posInt, aaString> entry is duplicated, it gets
                    // simply overwritten. this way redundant entries are
            ...
            if(threshold >= 4 && threshold < 5) {
                ...
                for(int j = 0; j < trimmedMegaList.get(12).size(); j++) {
                    ...
                    // fetch current position from ArrayList of positions
                    aaString = trimmedMegaList.get(13).get(j);
                    // fetch current AA from ArrayList of AAs
        ...
        if (!chain0Map.isEmpty()) {
            createFastaStrings(chain0IdString, chain0Map, resultsFileNameString);
        }
        if (!chain1Map.isEmpty()) {
            createFastaStrings(chain1IdString, chain1Map, resultsFileNameString);
        }
        if (!chain2Map.isEmpty()) {
            createFastaStrings(chain2IdString, chain2Map, resultsFileNameString);
        }
        if (!chain3Map.isEmpty()) {
            createFastaStrings(chain3IdString, chain3Map, resultsFileNameString);
        }
        if (!chain4Map.isEmpty()) {
            createFastaStrings(chain4IdString, chain4Map, resultsFileNameString);
        }
        if (!chain5Map.isEmpty()) {
            createFastaStrings(chain5IdString, chain5Map, resultsFileNameString);
        }
        if (!chain6Map.isEmpty()) {
            createFastaStrings(chain6IdString, chain6Map, resultsFileNameString);
        }
        if (!chain7Map.isEmpty()) {
            createFastaStrings(chain7IdString, chain7Map, resultsFileNameString);
        }
        if (!chain8Map.isEmpty()) {
            createFastaStrings(chain8IdString, chain8Map, resultsFileNameString);
        }
        if (!chain9Map.isEmpty()) {
            createFastaStrings(chain9IdString, chain9Map, resultsFileNameString);
        }
        if (!chain10Map.isEmpty()) {
            createFastaStrings(chain10IdString, chain10Map, resultsFileNameString);
        }
        if (!chain11Map.isEmpty()) {
            createFastaStrings(chain11IdString, chain11Map, resultsFileNameString);
        }
    }
    ...
    public static void createFastaStrings(String chainIdString, 
        ...
        // generate aaPositionsArray with length equal to the position of the
        ...
        //add a line break to te end of the AA sequence
